...
class Sentence(object):
    ...
    def Remove(self,index):
        result = self._symbols[0:index]
        self._symbols = self._symbols[index - 1,]
        return result
    
    def Insert(self,index,symbol):
        temp = list(self._symbols)
        if index == -1: temp.append(symbol)
        else : temp.insert(index,symbol)
        self._symbols = tuple(temp)
...
class SentenceList(object):
    ...
    def Remove(self,index):
        result = self._sentences[0:index]
        self._sentences = self._sentences[index - 1,]
        return result
    
    def Insert(self,index,symbol):
        if index == -1: self._sentences.append(symbol)
        else: self._sentences.insert(index,symbol)
